...
export const M3CTA = ({ label, url }: M3CTAProps) => {
  const classMap = {
    parent:
      "transition-padding-inline-end relative flex w-fit items-center justify-between gap-4 overflow-hidden rounded-2xl bg-[#903DF7] py-4 pe-6 ps-4 text-white duration-300 ease-in-out hover:pe-8 hover:text-white",
    child:
  ...
  return (
    <A1Link
      label={label}
      url={url}
      classList={`${classMap.parent} ${classMap.parentChild} ${classMap.parentChildMisc}`}
    >
